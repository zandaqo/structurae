...
test("[BinaryHeap.constructor] creates a new binary heap with given data", () => {
  ...
  assertEquals(Array.from(heap), [1, 3, 8, 10, 9, 20]);
});
...
test("[BinaryHeap#heapify] restores the binary heap", () => {
  ...
  heap[0] = 100;
  heap[1] = -2;
  assertEquals(Array.from(heap), [100, -2, 8, 10, 9, 20]);
  heap.heapify();
  assertEquals(Array.from(heap), [-2, 9, 8, 10, 100, 20]);
});
...
test("[BinaryHeap#replace] returns the first (min/max) element of the heap and replaces it with a given element", () => {
  ...
  assertEquals(head, 1);
  assertEquals(Array.from(heap), [0, 3, 8, 10, 9, 20]);
});
...
test("[BinaryHeap#shift] extracts the first element of the heap", () => {
  ...
  assertEquals(min, 1);
  assertEquals(Array.from(heap), [3, 9, 8, 10, 20]);
});
...
test("[BinaryHeap#splice] changes elements of the heap", () => {
  ...
  assertEquals(Array.from(heap), [3, 9, 8, 10, 20]);
  heap.splice(0, 0, 1);
  assertEquals(Array.from(heap), [1, 3, 9, 8, 10, 20]);
  heap.splice(4);
  assertEquals(Array.from(heap), [1, 3, 9, 8]);
  heap.push(4, 2, 7);
  assertEquals(Array.from(heap), [1, 3, 2, 8, 4, 9, 7]);
  const deleted = heap.splice(3, 2, 0, 6);
  assertEquals(Array.from(heap), [0, 1, 2, 3, 6, 9, 7]);
...
test("[BinaryHeap#unshift] adds elements to the heap", () => {
  const heap = new BinaryHeap(10, 1, 20, 3, 9, 8);
  heap.unshift(0, 12);
  assertEquals(Array.from(heap), [0, 3, 1, 10, 9, 20, 8, 12]);
});
...
test("[BinaryHeap#update] updates the position of an element inside the heap", () => {
  ...
  heap[1] = 21;
  heap.update(1);
  assertEquals(Array.from(heap), [1, 9, 8, 10, 21, 20]);
  heap[5] = -1;
  heap.update(5);
  assertEquals(Array.from(heap), [-1, 9, 1, 10, 21, 8]);
});
