...
test("[BitArray.create] creates a bit array of a required size", () => {
  const array = BitArray.create(100);
  assertEquals(array instanceof BitArray, true);
...
test("[BitArray#setBit] sets individual bits", () => {
  const array = BitArray.create(32);
  assertEquals(array.getBit(0), 0);
