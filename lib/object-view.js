...
/**
 * @typedef {Object} ObjectViewLayoutField
...
class ObjectView extends DataView {
   ...
   * @returns {void}
   */
  static initialize(ObjectViewClass = this) {
    const { schema } = this;
    const schemas = this.getSchemaOrdering(schema);
    for (let i = schemas.length - 1; i > 0; i--) {
      const id = schemas[i].$id;
      if (Reflect.has(ObjectView.Views, id)) continue;
      const View = class extends ObjectViewClass {};
      [View.layout, View.objectLength, View.fields] = this.getLayoutFromSchema(schemas[i]);
      View.setDefaultBuffer();
      ObjectView.Views[id] = View;
    }
    [this.layout, this.objectLength, this.fields] = this.getLayoutFromSchema(schema);
    ObjectView.Views[schema.$id] = this;
...
/**
 * @private
 * @type {Object<string, ObjectViewLayoutField>}
 */
