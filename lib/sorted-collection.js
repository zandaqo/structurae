...
function SortedMixin(Base) {
  ...
  class SortedCollection extends Base {
    ...
    range(start, end, subarray) {
      return this.constructor.getRange(this, start, end, this.compare, subarray);
    }
     ...
     * //=> [ 8, 3, 1 ]
     */
    static getDifference(a, b, symmetric, comparator = this.prototype.compare, container = []) {
      let i = 0;
      let j = 0;
     ...
     * //=> [ 4, 2 ]
     */
    static getIntersection(a, b, comparator = this.prototype.compare, container = []) {
      let i = 0;
      let j = 0;
      while (i < a.length && j < b.length) {
        const compared = comparator(a[i], b[j]);
     ...
     * //=> [ 9, 8, 7, 6, 4, 3, 2, 1 ]
     */
    static getUnion(a, b, unique, comparator = this.prototype.compare, container = []) {
      let i = 0;
      let j = 0;
      while (i < a.length && j < b.length) {
        const compared = comparator(a[i], b[j]);
     ...
     * //=> [ 1, 2, 3, 4 ]
     */
    static getUnique(arr, comparator = this.prototype.compare, container = []) {
      container[0] = arr[0];
