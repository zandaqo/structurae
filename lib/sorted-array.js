...
class SortedArray extends SortedMixin(Array) {
   ...
   */
  push(...elements) {
    const { getIndex } = this.constructor;
    const m = this.length;
    if (!m) return super.push(...elements.sort(this.compare));
    const toAdd = this.unique ? elements.filter(el => !~getIndex(this, el, this.compare))
      : elements;
    const n = toAdd.length;
    if (!n) return m;
    toAdd.sort(this.compare);
    for (let i = n - 1; i >= 0; i--) {
      let j;
      const last = this[m - 1];
      for (j = m - 2; j >= 0 && this.compare(this[j], toAdd[i]) === 1; j--) {
        this[j + 1] = this[j];
      }
      if (j !== m - 2 || this.compare(last, toAdd[i]) === 1) {
        this[j + 1] = toAdd[i];
        toAdd[i] = last;
      }
    }
