...
describe('BitField', () => {
  ...
  Field.schema = { width: 16, height: 15 };
  Field.initialize();
  ...
  describe('get', () => {
    it('returns a value of a given field', () => {
      expect(new Field({ width: 65535, height: 32767 }).get('width')).toBe(65535);
      expect(new Field({ width: 65535, height: 32767 }).get('height')).toBe(32767);
      expect(new PersonFlags({ human: 1, gender: 0, tall: 1 }).get('gender')).toBe(0);
  ...
  describe('has', () => {
    it('checks if all specified fields are set in a given bitfield instance', () => {
      expect(new PersonFlags({ human: 1, gender: 0, tall: 1 }).has('human', 'tall')).toBe(true);
      expect(new PersonFlags({ human: 1, gender: 1, tall: 1 }).has('human', 'tall')).toBe(true);
      expect(new PersonFlags({ human: 0, gender: 1, tall: 1 }).has('human', 'tall')).toBe(false);
      expect(new PersonFlags({ human: 1, gender: 1, tall: 0 }).has('human', 'tall')).toBe(false);
      expect(new PersonFlags({ human: 0, gender: 1, tall: 0 }).has('human', 'tall')).toBe(false);
      expect(new PersonFlags({ human: 0, gender: 1, tall: 0 }).has('gender')).toBe(true);
    });
  });
  ...
  describe('isValid', () => {
    ...
    it('checks if a given set of values is valued according to the schema', () => {
      ...
      expect(Person.isValid({ age: '', gender: '' })).toBe(false);
    });
  });
  ...
  describe('initialize', () => {
    ...
    it('throws if the total size of a field exceeds 31 bits', () => {
      class A extends BitField {}
      A.schema = { a: 1, b: 31 };
      expect(() => A.initialize()).toThrowError(TypeError);
    });
  });
});
