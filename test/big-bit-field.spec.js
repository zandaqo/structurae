...
describe('BigBitField', () => {
  ...
  describe('isValid', () => {
    it('checks if given pairs of Person name and value are valid according to the schema', () => {
      ...
      expect(LargeField.isValid({ location: -200 })).toBe(false);
      expect(LargeField.isValid({ location: 'asdf' })).toBe(false);
  ...
  describe('getMinSize', () => {
    it('calculates the minimum amount of bits to hold a given number', () => {
      for (let i = 1; i < 53; i++) {
        const number = 2 ** i;
        expect(BigBitField.getMinSize(number)).toEqual(i + 1);
        expect(BigBitField.getMinSize(number - 1)).toEqual(i);
      }
      expect(BigBitField.getMinSize(Number.MAX_SAFE_INTEGER)).toEqual(53);
    });
  });
