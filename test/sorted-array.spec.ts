...
describe("SortedArray", () => {
  ...
  beforeEach(() => {
    sorted = new SortedArray<number>(...first);
  });
  ...
  describe("isSorted", () => {
    it("checks if the array is sorted according to a provided comparator", () => {
      ...
      expect((SortedArray.isSorted as jest.Mock).mock.calls.length).toBe(1);
      expect((SortedArray.isSorted as jest.Mock).mock.calls[0]).toEqual([
        sorted,
      ]);
      (SortedArray.isSorted as jest.Mock).mockRestore();
    });
  });
  ...
  describe("isUnique", () => {
    it("checks if the array has duplicating elements", () => {
      ...
      expect((SortedArray.isUnique as jest.Mock).mock.calls.length).toBe(1);
      expect((SortedArray.isUnique as jest.Mock).mock.calls[0]).toEqual([
        sorted,
      ]);
      (SortedArray.isUnique as jest.Mock).mockRestore();
    });
  });

  describe("range", () => {
    it("returns range", () => {
      expect(sorted.range(2, 4)).toEqual(SortedArray.from([2, 3, 4]));
      expect(sorted.range(2)).toEqual(SortedArray.from([2, 3, 4, 8]));
      expect(sorted.range(undefined, 4)).toEqual(
        SortedArray.from([1, 2, 3, 4])
      );
    });
  });
  ...
  describe("from", () => {
    it("creates a sorted array from an array-like object", () => {
      expect(SortedArray.from([2, 1, 5, 3, 80, 9])).toEqual(
  ...
  describe("getDifference", () => {
    ...
    it("returns the symmetric difference if `symmetric=true`", () => {
      expect(SortedArray.getDifference(first, second, true)).toEqual([
        1,
        3,
        6,
        7,
  ...
  describe("getUnion", () => {
    it("returns the union of two sorted arrays", () => {
      expect(SortedArray.getUnion(first, second)).toEqual([
        1,
        2,
        2,
        3,
    ...
    it("returns the union of two arrays without duplicates if `unique=true`", () => {
      expect(SortedArray.getUnion(first, second, true)).toEqual([
        1,
        2,
        3,
        4,
  ...
  describe("getUnique", () => {
    ...
    it("returns the unique values in a given container", () => {
      ...
      expect(SortedArray.getUnique([1, 1, 2, 3, 3, 4], undefined, [])).toEqual([
        1,
        2,
        3,
        4,
  ...
  describe("of", () => {
    it("creates a sorted array from provided arguments", () => {
      expect(SortedArray.of(2, 1, 5, 3, 80, 9)).toEqual(
        new SortedArray(1, 2, 3, 5, 9, 80)
      );
      expect(SortedArray.of(2)).toEqual(SortedArray.from([2]));
    });
  });
  ...
  describe("concat", () => {
    it("returns a sorted combination of two arrays", () => {
      expect(Array.from(sorted.concat(second))).toEqual([
        1,
        2,
        2,
        3,
  ...
  describe("push", () => {
    ...
    it("adds multiple items to the array preserving the order", () => {
      ...
      expect(Array.from(sorted)).toEqual([
        0,
        1,
        1,
        1,
  ...
  describe("set", () => {
    it("replaces the elements of the array", () => {
      expect(Array.from(sorted.set(second))).toEqual(second);
    });
  });

  describe("splice", () => {
    it("removes specified amount of elements starting from a given index", () => {
      expect(Array.from(sorted.splice(0, 2))).toEqual([1, 2]);
      expect(Array.from(sorted)).toEqual([3, 4, 8]);
    });

    it("adds new elements preserving order", () => {
      expect(Array.from(sorted.splice(1, 1, 3, 4))).toEqual([2]);
      expect(Array.from(sorted)).toEqual([1, 3, 3, 4, 4, 8]);
    });
  });
  ...
  describe("uniquify", () => {
    it("removes duplicates from the array", () => {
      ...
      unique.uniquify();
      expect(Array.from(unique)).toEqual([1, 2, 3, 4, 5, 9]);
    });
  });
  ...
  describe("unshift", () => {
    it("adds elemets to the array preserving order", () => {
      ...
      expect(Array.from(sorted)).toEqual([0, 1, 2, 3, 4, 5, 8, 9]);
    });
  });
});
