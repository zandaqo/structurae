...
describe('SortedArray', () => {
  ...
  describe('reset', () => {
    it('replaces the elements of the array', () => {
      expect(sorted.reset(second)).toEqual(second);
    });
  });
  ...
  describe('getRank', () => {
    it('returns the rank of an element in a sorted array', () => {
      expect(SortedArray.getRank(second, 5)).toBe(2);
    });

    it('uses custom comparator if provided', () => {
      expect(SortedArray.getRank(inversedSecond, 5, customComparator)).toBe(3);
    });
  });
